...
Output:
  - data/processed/member_b/member_b_final.parquet  -> assembled panel
...
def main():
    ...
    # Save final dataset
    parquet_path = os.path.join(OUTPUT_DIR, "member_b_final.parquet")
    panel.to_parquet(parquet_path, index=False, engine="pyarrow")
    print(f"\n[OUTPUT] {panel.shape} -> {parquet_path}")

    # Also save as CSV for consistency with member_a/c
    csv_path = os.path.join(OUTPUT_DIR, "member_b_final.csv")
    panel.to_csv(csv_path, index=False)
    print(f"[OUTPUT] {panel.shape} -> {csv_path}")
